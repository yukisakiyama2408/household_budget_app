...
export default function CsvImportForm({ categories }: { categories: Category[] }) {
    ...
    reader.onload = (e) => {
      ...
      Papa.parse<Record<string, string>>(text, {
        ...
        complete: ({ data, meta }) => {
          ...
          const dateCol = detectColumn(headers, ["取引日時", "取引日", "日付", "日時"]);
          const expenseCol = detectColumn(headers, ["出金金額", "取引金額", "利用金額", "支払金額"]);
          const incomeCol = detectColumn(headers, ["入金金額"]);
          const typeCol = detectColumn(headers, ["取引内容"]);
          const storeCol = detectColumn(headers, ["取引先名", "取引先", "取引相手", "店舗名"]);
          const fallbackContentCol = !storeCol ? detectColumn(headers, ["内容"]) : null;

          if (!dateCol || (!expenseCol && !incomeCol)) {
            setError(`日付・金額の列が見つかりませんでした。\n検出された列: ${headers.join(", ")}`);
            return;
          }

          const SKIP_TYPES = ["チャージ", "ポイント"];
          const INCOME_TYPES = ["受け取った金額"];
          ...
          for (const row of data) {
            ...
            const expenseRaw = expenseCol ? row[expenseCol]?.trim() : "";
            const incomeRaw = incomeCol ? row[incomeCol]?.trim() : "";
            const expenseAmount = expenseRaw && expenseRaw !== "-" ? parseAmount(expenseRaw) : 0;
            const incomeAmount = incomeRaw && incomeRaw !== "-" ? parseAmount(incomeRaw) : 0;

            if (expenseAmount === 0 && incomeAmount === 0) continue;

            const isIncome =
              INCOME_TYPES.some((s) => txType.includes(s)) ||
              (incomeAmount > 0 && expenseAmount === 0);
            const type: "income" | "expense" = isIncome ? "income" : "expense";
            const amount = isIncome ? incomeAmount : expenseAmount;

            const storeName =
              (storeCol ? row[storeCol]?.trim() : null) ||
              (fallbackContentCol ? row[fallbackContentCol]?.trim() : null) ||
              null;

            parsed.push({
              date: parseDate(dateRaw),
              content: storeName || (type === "expense" ? "PayPay支払い" : "PayPay入金"),
              amount,
              type,
              category_id: null,
              pay_method: "Cash",
              ...
              <tbody>
                {filteredIndexedRows.map(({ row, originalIndex }) => {
                  ...
                  <tr key={originalIndex} className={`border-b ${dupLevel === "high" ? "bg-orange-50" : dupLevel === "low" ? "bg-yellow-50" : "hover:bg-gray-50"}`}>
                    ...
                    <td className="py-2 pr-3 text-gray-500 whitespace-nowrap">Cash</td>
                    <td className="py-2 pr-3">
                      <input
